...
from talkpipe.util.os import SecurityError, run_command


def _compile_and_run(expression):
    """Compile an expression and evaluate it against a dummy item.

    The security checks may reject an expression either at compile time or
    when it is evaluated; the tests only care that it is rejected at one of
    the two points.
    """
    return compileLambda(expression)({"test": "data"})
...
class TestUnsafeEvalExecution:
    ...
    def test_eval_code_injection_attack(self):
        ...
        with pytest.raises((ValueError, NameError, AttributeError)) as exc_info:
            _compile_and_run(malicious_expression)
        ...
        assert (
            ...
            or "prohibited pattern" in error_msg
        )
    ...
    def test_eval_file_system_access_blocked(self):
        ...
        assert (
            "security violation" in error_msg
            or "'open' is not defined" in error_msg
            or "not allowed" in error_msg
            or "prohibited pattern" in error_msg
        )
...
class TestShellInjectionVulnerability:
    ...
    def test_command_injection_blocked(self):
        ...
        with pytest.raises(
            (subprocess.CalledProcessError, ValueError, SecurityError)
        ) as exc_info:
        ...
        assert (
            ...
            or exc_info.typename in ["SecurityError", "ValueError"]
        )
...
class TestCORSAndAuthenticationWeaknesses:
    ...
    def test_cors_origin_restrictions(self):
        ...
        # This test should FAIL before security fix
        server = ChatterlangServer(host="127.0.0.1", port=9998, require_auth=False)

        client = TestClient(server.app)

        # Test that wildcard CORS is NOT allowed
        response = client.options(
            "/process", headers={"Origin": "http://malicious-site.com"}
        )
    ...
    def test_authentication_required_for_sensitive_endpoints(self):
        ...
        server = ChatterlangServer(
            host="127.0.0.1", port=9999, api_key="secure-key", require_auth=True
        )
    ...
    def test_server_sent_events_authentication(self):
        ...
        server = ChatterlangServer(
            host="127.0.0.1", port=10000, api_key="secure-key", require_auth=True
        )
    ...
    def test_session_cookie_security(self):
        ...
        server = ChatterlangServer(host="127.0.0.1", port=10001, require_auth=False)
