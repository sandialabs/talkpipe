...
def test_get_robot_parser_exceptions():
    ...
    with (
        ...
        patch.object(RobotFileParser, "parse", side_effect=Exception("Parse error")),
    ):
...
def test_downloadURL_exceptions():
    ...
    with (
        patch("talkpipe.data.html.can_fetch", return_value=True),
        patch("requests.get", return_value=mock_response),
    ):
    ...
    with (
        patch("talkpipe.data.html.can_fetch", return_value=True),
        patch("talkpipe.data.html.time.sleep"),
        patch("requests.get", side_effect=requests.Timeout("Request timed out")),
    ):
        # With fail_on_error=True, should raise Exception
        with pytest.raises(Exception, match="Failed to download URL"):
            html.downloadURL(
                "https://example.com/timeout", fail_on_error=True, timeout=1
            )

        # With fail_on_error=False, should return None
        result = html.downloadURL(
            "https://example.com/timeout", fail_on_error=False, timeout=1
        )
        assert result is None
...
def test_downloadURL_retries_transient_connection_error():
    ...
    with (
        patch("talkpipe.data.html.can_fetch", return_value=True),
        patch("talkpipe.data.html.time.sleep") as mock_sleep,
        patch(
            "requests.get", side_effect=[requests.ConnectionError("reset"), ok]
        ) as mock_get,
    ):
...
def test_downloadURL_retries_transient_status_codes():
    ...
    with (
        ...
        patch("talkpipe.data.html.time.sleep"),
        patch(
            "requests.get", side_effect=[_mock_response(status_code=503), ok]
        ) as mock_get,
    ):
...
def test_downloadURL_honors_retry_after():
    ...
    with (
        patch("talkpipe.data.html.can_fetch", return_value=True),
        patch("talkpipe.data.html.time.sleep") as mock_sleep,
        patch("requests.get", side_effect=[throttled, ok]),
    ):
...
def test_downloadURL_does_not_retry_permanent_failures():
    ...
    with (
        patch("talkpipe.data.html.can_fetch", return_value=True),
        patch("talkpipe.data.html.time.sleep") as mock_sleep,
        patch("requests.get", return_value=_mock_response(status_code=404)) as mock_get,
    ):
...
def test_downloadURL_exhausts_retries_on_persistent_5xx():
    ...
    with (
        patch("talkpipe.data.html.can_fetch", return_value=True),
        patch("talkpipe.data.html.time.sleep"),
        patch("requests.get", return_value=_mock_response(status_code=500)) as mock_get,
...
def test_downloadURL_accepts_any_2xx_status():
    ...
    with (
        patch("talkpipe.data.html.can_fetch", return_value=True),
        patch("requests.get", return_value=_mock_response(status_code=203)),
    ):
...
def test_downloadURL_sends_browser_headers():
    ...
    with (
        patch("talkpipe.data.html.can_fetch", return_value=True),
        patch("talkpipe.data.html.get_config", return_value={}),
        patch("requests.get", return_value=_mock_response()) as mock_get,
    ):
