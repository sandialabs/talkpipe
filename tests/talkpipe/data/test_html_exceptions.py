...
class TestHtmlModuleExceptions:
    ...
    def test_downloadURL_disallowed_by_robots(self, mock_can_fetch):
        ...
        with pytest.raises(PermissionError, match=r"disallowed by robots\.txt"):
        ...
        mock_can_fetch.assert_called_with(
            "https://example.com", html.DEFAULT_USER_AGENT
        )
    ...
    def test_downloadURL_non_200_status(self, mock_get, mock_can_fetch):
        ...
        # Test with fail_on_error=True
        with pytest.raises(
            ValueError, match=r"Failed to download URL.*with status code 404"
        ):
            downloadURL("https://example.com", fail_on_error=True)
    ...
    def test_downloadURLSegment_exception(self, mock_get):
        ...
        with pytest.raises(
            ValueError,
            match=r"Failed to download URL: https://example\.com with status code 404",
        ):
    ...
    def test_can_fetch_with_robots_txt_exception(self, mock_get_robot_parser):
        ...
        mock_parser.can_fetch.assert_called_once_with(
            html.DEFAULT_USER_AGENT, "https://example.com"
        )
