...
def test_shingle_generator_count_mode():
    ...
    # Test with overlap
    result = list(
        ops.shingle_generator(
            items, string_field="text", key_field="key", shingle_size=3, overlap=2
        )
    )
    ...
    assert shingles == expected_shingles, (
        f"Expected {expected_shingles}, got {shingles}"
    )

    # Test with no overlap
    result_no_overlap = list(
        ops.shingle_generator(
            items, string_field="text", key_field="key", shingle_size=4, overlap=0
        )
    )
    ...
    assert shingles_no_overlap == expected_shingles_no_overlap, (
        f"Expected {expected_shingles_no_overlap}, got {shingles_no_overlap}"
    )
...
def test_shingle_generator_length_mode():
    ...
    assert shingles_no_overlap == expected_no_overlap, (
        f"Expected {expected_no_overlap}, got {shingles_no_overlap}"
    )
...
def test_shingle_generator_no_field():
    ...
    result = list(
        ops.shingle_generator(
            words, string_field=None, key_field=None, shingle_size=2, overlap=1
        )
    )
...
def test_shingle_generator_with_paragraph_numbers():
    ...
    assert len(first_result) == 4, (
        f"Expected 4-tuple (item, text, first_para, last_para), got {len(first_result)}-tuple"
    )

    _item, text, first_para, last_para = first_result
    ...
    _item, text, first_para, last_para = middle_result
    ...
    _item, text, first_para, last_para = key2_result
    assert text == "The rain in", f"Expected 'The rain in', got '{text}'"
    assert first_para == 0, (
        f"Expected first_para=0 (counter should reset on key change), got {first_para}"
    )
    assert last_para == 2, (
        f"Expected last_para=2 (counter should reset on key change), got {last_para}"
    )
    ...
    assert len(result_default[0]) == 2, (
        f"Expected 2-tuple by default, got {len(result_default[0])}-tuple"
    )
...
def test_shingle_generator_single_incomplete_shingle_with_overlap():
    ...
    result = list(
        ops.shingle_generator(
            items, string_field="text", key_field="key", shingle_size=3, overlap=1
        )
    )
    ...
    items = [{"key": 1, "text": "hello"}, {"key": 2, "text": "world"}]
