...
def test_mine_simple_pipeline():
    chains = corpus.mine_script(
        'INPUT FROM echo[data="hi"] | cast[cast_type="int"] | print'
    )
    assert chains == [["echo", "cast", "print"]]


def test_mine_multiple_pipelines():
    chains = corpus.mine_script('INPUT FROM echo[data="1"] | @x; INPUT FROM @x | print')
...
def test_corpus_tables_weight_workspace_scripts():
    ...
    gained = with_user["bigrams"]["echo"].get("firstN", 0) - without_user[
        "bigrams"
    ].get("echo", {}).get("firstN", 0)
