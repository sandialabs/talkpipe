...
def test_stats_reflect_saved_pipelines(client):
    ...
    client.post(
        ...
        json={
            "name": "Mine",
            "script": "INPUT FROM echo | firstN[n=2]",
        },
    )
    after = client.get("/api/suggest/stats").json()
    gained = after["bigrams"].get("echo", {}).get("firstN", 0) - before["bigrams"].get(
        "echo", {}
    ).get("firstN", 0)
    assert gained >= 3
...
def test_suggest_happy_path(with_fake_llm):
    response = with_fake_llm.post(
        "/api/suggest",
        json={
            "script": "INPUT FROM echo |",
            "cursor_offset": 17,
        },
    )
    data = response.json()
...
def test_settings_roundtrip(with_fake_llm, tmp_path):
    ...
    response = with_fake_llm.put(
        "/api/settings",
        json={
            "suggest_source": "fake",
            "suggest_model": "other-model",
            "auto_suggest": True,
        },
    )
    data = response.json()
...
def test_prompt_prefers_user_pipelines_over_builtin(with_fake_llm):
    with_fake_llm.post(
        "/api/pipelines",
        json={"name": "Mine A", "script": 'INPUT FROM echo[data="1"] | print'},
    )
    with_fake_llm.post(
        "/api/pipelines",
        json={
            "name": "Mine B",
            "script": 'INPUT FROM echo[data="2"] | cast[cast_type="int"]',
        },
    )
    with_fake_llm.post(
        "/api/pipelines",
        json={
            "name": "Mine C",
            "script": 'INPUT FROM echo[data="3"] | @x; INPUT FROM @x | print',
        },
    )
    with_fake_llm.post(
        "/api/suggest",
