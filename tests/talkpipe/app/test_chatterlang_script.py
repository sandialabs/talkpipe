...
def test_warning_messages_should_not_include_full_script_content(caplog):
    ...
    """
    test_script = 'INPUT FROM echo[data="test1,test2,test3"] ' + "".join(
        ["| print " for _ in range(100)]
    )  # Long script

    with (
        caplog.at_level(logging.WARNING),
        patch("argparse.ArgumentParser.parse_known_args") as mock_args,
    ):
        mock_namespace = type("MockNamespace", (), {})()
        mock_namespace.script = test_script
        mock_namespace.load_module = []
...
def test_runtime_error_prints_concise_message(capsys):
    ...
    """
    test_script = (
        'INPUT FROM "this_file_does_not_exist_12345.jsonl" | readJsonl | print'
    )
