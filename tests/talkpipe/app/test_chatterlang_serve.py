...
class TestChatterlangServer:
    ...
    def test_init_custom_config(self, server_config, form_config):
        ...
        server = ChatterlangServer(**server_config, form_config=form_config)
...
class TestChatterlangServerEndpoints:
    ...
    def server(self):
        ...
        return ChatterlangServer(
            host="localhost", port=8999, api_key="test-key", require_auth=False
        )
    ...
    def test_process_endpoint_with_auth(self):
        ...
        response = client.post(
            "/process", json=test_data, headers={"X-API-Key": "test-secret"}
        )
        assert response.status_code == 200

        # Test with incorrect API key
        response = client.post(
            "/process", json=test_data, headers={"X-API-Key": "wrong-key"}
        )
        assert response.status_code == 403
    ...
    def test_history_endpoint(self, client, server):
        ...
        response = client.get(
            "/history?limit=10", cookies={"talkpipe_session_id": "test-session"}
        )
    ...
    def test_clear_history_endpoint(self, client, server):
        ...
        response = client.delete(
            "/history", cookies={"talkpipe_session_id": "test-session"}
        )
...
class TestHTMLGeneration:
    ...
    def test_generate_form_fields_select(self):
        ...
        server = ChatterlangServer()
        field = FormField(
            name="choice", type="select", options=["opt1", "opt2", "opt3"]
        )
        server.form_config.fields = [field]
    ...
    def test_generate_form_fields_textarea(self):
        ...
        server = ChatterlangServer()
        field = FormField(
            name="description", type="textarea", rows=5, default="Default text"
        )
        server.form_config.fields = [field]
...
class TestChatterlangServerSegment:
    ...
    def test_init(self, mock_server_class):
        ...
        segment = ChatterlangServerSegment(
            port=8888, host="127.0.0.1", api_key="test-key", require_auth=True
        )
...
class TestMainFunction:
    ...
    @patch("talkpipe.app.chatterlang_serve.load_script")
    @patch("sys.argv", ["script.py", "--script", "test_script.cl"])
    def test_go_with_uncompilable_script_exits(
        self, mock_load_script, mock_server_class, capsys
    ):
    ...
    @patch(
        "sys.argv",
        ["script.py", "--CUSTOM_VAR", "custom_value", "--ANOTHER_VAR", "123"],
    )
    def test_go_with_constants(self, mock_server_class):
        ...
        with (
            ...
            mock_add_config.assert_called_once_with(
                {"CUSTOM_VAR": "custom_value", "ANOTHER_VAR": "123"}, override=True
            )
    ...
    def test_go_api_key_cmdline_overrides_config(
        self, mock_get_config, mock_server_class
    ):
...
class TestAsyncFunctionality:
    ...
    def test_process_json_success(self):
        ...
        result = server._process_json(test_data, session)
...
class TestServerStop:
    """``stop()`` must actually shut down a background server."""

    def test_stop_signals_uvicorn_and_joins_thread(self, monkeypatch):
        import threading

        import talkpipe.app.chatterlang_serve as serve_mod
        ...
        monkeypatch.setattr(serve_mod.uvicorn, "Server", FakeServer)
        monkeypatch.setattr(serve_mod.uvicorn, "Config", lambda app, **kw: kw)
        server = ChatterlangServer(port=0)
        monkeypatch.setattr(server, "_check_port_available", lambda: None)
