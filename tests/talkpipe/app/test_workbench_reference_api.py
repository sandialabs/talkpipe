...
def test_lint_full_mode_regenerates_compile_error_diagnostics(client, monkeypatch):
    # A CompileError's own text is never echoed to the client (information
    # exposure through an exception); the diagnostics are regenerated by the
    # static parse-mode pass instead.
    def fake_compile(script):
        raise CompileError(
            "secret /internal/path leaked", kind="unknown_name", bad_name="something"
        )
    ...
    diagnostics = response.json()["diagnostics"]
    assert len(diagnostics) == 1
    assert "secret" not in diagnostics[0]["message"]
    assert "'something' not found" in diagnostics[0]["message"]
    assert diagnostics[0]["kind"] == "unknown_name"


def test_lint_full_mode_opaque_compile_error_gets_located_pointer(client, monkeypatch):
    # A compile failure the static pass can't reproduce (e.g. a constructor
    # rejecting a parameter value) still yields a located diagnostic, without
    # the exception text.
    def fake_compile(script):
        raise CompileError(
            "constructor blew up at /some/path", line=1, column=3, kind="bad_param"
        )

    monkeypatch.setattr(reference_api.chatterlang_compiler, "compile", fake_compile)
    response = client.post(
        "/api/lint",
        json={"script": 'INPUT FROM echo[data="x"] | print', "mode": "full"},
    )
    diagnostics = response.json()["diagnostics"]
    assert len(diagnostics) == 1
    d = diagnostics[0]
    assert "/some/path" not in d["message"]
    assert "Compilation failed" in d["message"]
    assert (d["line"], d["column"]) == (1, 3)
    assert d["kind"] == "bad_param"


def test_lint_full_mode_internal_error_is_not_echoed(client, monkeypatch):
    def fake_compile(script):
        raise RuntimeError("KeyError at /home/user/.secrets/config")
    ...
    diagnostics = response.json()["diagnostics"]
    assert len(diagnostics) == 1
    assert ".secrets" not in diagnostics[0]["message"]
