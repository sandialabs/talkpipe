...
from talkpipe.util.config import load_script, reset_config
...
with multiple lines and special characters like !@#$%^&*()_+.
    ...
    def test_load_script_from_file_utf8(self):
        ...
        with tempfile.NamedTemporaryFile(
            mode="w", suffix=".tp", delete=False, encoding="utf-8"
        ) as f:
            f.write(script_content)
            temp_path = f.name
    ...
    def test_load_script_file_not_readable(self):
        ...
        if os.getuid() == 0:
            pytest.skip(
                "Test skipped when running as root - root bypasses file permissions"
            )
    ...
    def test_load_script_from_environment_variable(self):
        ...
        with (
            patch("os.environ", {"TALKPIPE_TEST_VAR": script_content}),
    ...
    def test_load_script_file_path_with_spaces(self):
        ...
        with tempfile.NamedTemporaryFile(
            mode="w", suffix=" test file.tp", delete=False
        ) as f:
            f.write(script_content)
            temp_path = f.name
...
class TestLoadModuleFile:
    ...
    def test_missing_file_raises_file_not_found(self):
        ...
        with pytest.raises(FileNotFoundError, match=r"no_such_module\.py"):
    ...
    def test_missing_file_returns_none_by_default(self):
        ...
        assert (
            load_module_file(fname="no_such_module.py", fail_on_missing=False) is None
        )
