...
def test_configure_logging_files_only(tmp_path):
    """logger_files without logger_levels must not fail and must log at the
    logger's effective level."""
    log_file = tmp_path / "files_only.log"
    logger_name = "talkpipe.test.files_only"
    logging.getLogger(logger_name).setLevel(logging.INFO)

    talkpipe.util.config.configure_logger(logger_files=f"{logger_name}:{log_file}")

    logger = logging.getLogger(logger_name)
    logger.debug("files-only debug message")
    logger.info("files-only info message")
    for handler in logger.handlers:
        handler.flush()
    contents = log_file.read_text()
    assert "files-only info message" in contents
    assert "files-only debug message" not in contents


def test_talkpipe_logger_has_null_handler():
    import logging

    import talkpipe  # noqa: F401

    handlers = logging.getLogger("talkpipe").handlers
    assert any(isinstance(h, logging.NullHandler) for h in handlers)


def test_get_config_concurrent_first_load_is_consistent(monkeypatch, tmp_path):
    import threading

    from talkpipe.util import config as config_module

    monkeypatch.setenv("TALKPIPE_CONCURRENT_PROBE", "yes")
    ...
    def worker():
        barrier.wait()
        results.append(config_module.get_config(path=str(tmp_path / "none.toml")))
    ...
    assert results[0]["CONCURRENT_PROBE"] == "yes"
    config_module.reset_config()
