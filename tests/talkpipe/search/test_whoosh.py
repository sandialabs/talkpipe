...
def test_indexWhoosh(temp_index_dir):
    ...
    f = compile(
        f"""| indexWhoosh[field_list="title,content,value", index_path="{temp_index_dir}"]"""
    )
    results = list(f(data))
    ...
    f = compile(
        f"""| indexWhoosh[field_list="title,content,value", index_path="{temp_index_dir}", yield_doc=True]"""
    )
    results = list(f(data))
    assert len(results) == len(data)
...
def test_searchWhoosh(temp_index_dir):
    ...
    f = compile(
        f"""| indexWhoosh[field_list="title,content", index_path="{temp_index_dir}"]"""
    )
...
def test_WhooshWriter_commit_and_rollback(temp_index_dir, index_fields):
    ...
    try:
        with WhooshWriter(temp_index_dir, index_fields) as idx:
            idx.add_document(
                {"title": "ExceptionDoc", "content": "Should NOT be committed"}
            )
...
def test_WhooshWriter_and_WhooshSearcher_integration(temp_index_dir, index_fields):
    ...
    with WhooshSearcher(temp_index_dir) as idx_search:
        ...
        assert "Integration1" in titles
        assert "Integration2" in titles
...
def test_update_document(temp_index_dir, index_fields):
    ...
    idx = WhooshFullTextIndex(temp_index_dir, index_fields)
    # Update the document
    success = idx.update_document(
        doc_id, {"title": "Updated", "content": "Updated content"}
    )
...
def test_update_nonexistent_document(temp_index_dir, index_fields):
    ...
    idx = WhooshFullTextIndex(temp_index_dir, index_fields)
    success = idx.update_document(
        "nonexistent-id", {"title": "Won't work", "content": "Nope"}
    )
...
def test_indexWhoosh_continue_on_error(temp_index_dir):
    ...
    class BadItem:
        def __init__(self):
            self.title = "Bad"
            self.content = (
                self  # This will cause infinite recursion or error during str()
            )
    ...
    f = compile(
        f"""| indexWhoosh[field_list="title,content", index_path="{temp_index_dir}", continue_on_error=True]"""
    )
    results = list(f(data))
...
def test_indexWhoosh_stop_on_error(temp_index_dir):
    ...
    f = compile(
        f"""| indexWhoosh[field_list="title,content", index_path="{temp_index_dir}", continue_on_error=False]"""
    )
...
def test_searchWhoosh_with_set_as_and_all_results_at_once(temp_index_dir):
    ...
    f = compile(
        f"""| searchWhoosh[index_path="{temp_index_dir}", field="query", set_as="results", all_results_at_once=True]"""
    )
...
def test_searchWhoosh_set_as_without_all_results_at_once_raises_error(temp_index_dir):
    ...
    queries = [{"query": "Test"}]
    f = compile(
        f"""| searchWhoosh[index_path="{temp_index_dir}", field="query", set_as="results", all_results_at_once=False, continue_on_error=False]"""
    )

    with pytest.raises(
        ValueError,
        match="set_as only works with this segment if all_results_at_once is True",
    ):
        list(f(queries))
...
def test_searchWhoosh_continue_on_error(temp_index_dir):
    ...
    f = compile(
        f"""| searchWhoosh[index_path="{temp_index_dir}", continue_on_error=True]"""
    )
    results = list(f(queries))
...
def test_searchWhoosh_stop_on_error(temp_index_dir):
    ...
    queries = ["Test"]
    f = compile(
        f"""| searchWhoosh[index_path="{nonexistent_path}", continue_on_error=False]"""
    )
...
def test_searchWhoosh_yield_from_results(temp_index_dir):
    ...
    f = compile(
        f"""| searchWhoosh[index_path="{temp_index_dir}", all_results_at_once=False]"""
    )
...
def test_searchWhoosh_all_results_at_once_without_set_as(temp_index_dir):
    ...
    f = compile(
        f"""| searchWhoosh[index_path="{temp_index_dir}", all_results_at_once=True]"""
    )
    results = list(f(["Test"]))
...
def test_searchWhoosh_flush_triggers_reload(temp_index_dir):
    ...
    with mock.patch("talkpipe.search.whoosh.WhooshSearcher") as mock_searcher_class:
        ...
        mock_searcher.text_search.return_value = [
            SearchResult(doc_id="1", score=1.0, document={"title": "Test"})
        ]
...
def test_indexWhoosh_flush_makes_documents_searchable_immediately(temp_index_dir):
    ...
    assert len(committed_results) == 1, (
        "Document should be searchable immediately after Flush commit"
    )
    ...
    assert len(uncommitted_results) == 0, (
        "Document should NOT be searchable before context manager exits (no Flush)"
    )

    # Finish the generator to commit everything (exhausting it commits on
    # context exit; any error from the remaining items is irrelevant here)
...
def test_writer_wrapper_append_commits_skip_merging(temp_index_dir):
    ...
    with WhooshWriter(temp_index_dir, ["title", "content"], commit_seconds=0) as idx:
        ...
        for i in range(10):
            idx.add_document(
                {"title": f"Doc {i}", "content": f"content {i}"}, upsert=False
            )
...
def test_writer_wrapper_upsert_commits_merge(temp_index_dir):
    ...
    with WhooshWriter(temp_index_dir, ["title", "content"], commit_seconds=0) as idx:
        ...
        for i in range(5):
            idx.add_document(
                {"title": f"Doc {i}", "content": f"content {i}"}, doc_id=f"doc-{i}"
            )
            track_writer(idx)
...
def test_indexWhoosh_frequent_commits_preserve_upserts(temp_index_dir):
    ...
    frequent commits."""
    items = [
        {"doc_id": f"doc-{i}", "title": f"Doc {i}", "content": "original"}
        for i in range(5)
    ] + [
        {"doc_id": f"doc-{i}", "title": f"Doc {i}", "content": "replaced"}
        for i in range(5)
    ]
    seg = indexWhoosh(
        index_path=temp_index_dir,
