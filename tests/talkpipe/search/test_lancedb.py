...
def sample_search_results():
    ...
    return [
        ...
        SearchResult(
            score=0.8, doc_id="doc2", document={"text": "another similar item"}
        ),
    ]
...
class TestSearchLanceDB:
    def test_search_lancedb_missing_path_raises_error(self, sample_items):
        ...
        with pytest.raises(
            ValueError, match="Both 'path' and 'table' parameters must be provided"
        ):
            list(seg(sample_items))

    def test_search_lancedb_missing_table_raises_error(
        self, sample_items, temp_db_path
    ):
    ...
    def test_search_lancedb_with_field(
        self,
        ...
        seg = search_lancedb(
            path=temp_db_path, table_name="test_table", field="vector", limit=5
        )
        results = list(seg(items_to_search))

        mock_doc_store_class.assert_called_once_with(
            temp_db_path, "test_table", None, 10
        )
    ...
    @mock.patch("talkpipe.search.lancedb.LanceDBDocumentStore")
    def test_search_lancedb_without_field(
        self, mock_doc_store_class, sample_search_results, temp_db_path
    ):
        mock_doc_store = mock.Mock()
        mock_doc_store_class.return_value = mock_doc_store
        mock_doc_store.vector_search.return_value = sample_search_results
    ...
    @mock.patch("talkpipe.search.lancedb.LanceDBDocumentStore")
    @mock.patch("talkpipe.search.lancedb.extract_property")
    def test_search_lancedb_all_results_at_once(
        self,
        mock_extract_property,
        mock_doc_store_class,
        sample_items,
    ...
    @mock.patch("talkpipe.search.lancedb.LanceDBDocumentStore")
    @mock.patch("talkpipe.search.lancedb.extract_property")
    def test_search_lancedb_set_as(
        self,
        mock_extract_property,
        mock_doc_store_class,
        sample_items,
        ...
        assert (
            len(results) == 1
        )  # Only yields the original item with search_results attached

    @mock.patch("talkpipe.search.lancedb.LanceDBDocumentStore")
    @mock.patch("talkpipe.search.lancedb.extract_property")
    def test_search_lancedb_multiple_items(
        self,
        mock_extract_property,
        mock_doc_store_class,
        sample_items,
    ...
    @mock.patch("talkpipe.search.lancedb.LanceDBDocumentStore")
    def test_search_lancedb_default_limit(
        self, mock_doc_store_class, sample_search_results, temp_db_path
    ):
        mock_doc_store = mock.Mock()
        mock_doc_store_class.return_value = mock_doc_store
        ...
        mock_doc_store.vector_search.assert_called_once_with(
            [7.0, 8.0, 9.0], 10
        )  # Default limit
...
class TestAddToLanceDB:
    ...
    def test_add_to_lancedb_missing_table_raises_error(
        self, sample_items, temp_db_path
    ):
    ...
    @mock.patch("talkpipe.search.lancedb.LanceDBDocumentStore")
    @mock.patch("talkpipe.search.lancedb.extract_property")
    def test_add_to_lancedb_with_vector_field(
        self, mock_extract_property, mock_doc_store_class, sample_items, temp_db_path
    ):
        mock_doc_store = mock.Mock()
        ...
        seg = add_to_lancedb(
            path=temp_db_path, table_name="test_table", vector_field="vector"
        )
        results = list(seg(items_to_add))

        mock_doc_store_class.assert_called_once_with(temp_db_path, "test_table", None)
        mock_extract_property.assert_called_once_with(
            sample_items[0], "vector", fail_on_missing=True
        )
    ...
    @mock.patch("talkpipe.search.lancedb.LanceDBDocumentStore")
    @mock.patch("talkpipe.search.lancedb.extract_property")
    def test_add_to_lancedb_with_overwrite(
        self, mock_extract_property, mock_doc_store_class, sample_items, temp_db_path
    ):
        mock_doc_store = mock.Mock()
        mock_doc_store._get_table.return_value = [mock.Mock()]
    ...
    @mock.patch("talkpipe.search.lancedb.LanceDBDocumentStore")
    def test_add_to_lancedb_skips_zero_vectors(
        self, mock_doc_store_class, temp_db_path
    ):
        mock_doc_store = mock.Mock()
        mock_doc_store_class.return_value = mock_doc_store
        ...
        items = [
            {
                "vector": [0.0, 0.0, 0.0],
                "text": "all-unknown-token content (e.g. base64)",
            },
        ...
        added_rows = [
            row
            for call in mock_doc_store.add_vectors.call_args_list
    ...
    @mock.patch("talkpipe.search.lancedb.LanceDBDocumentStore")
    def test_add_to_lancedb_keeps_zero_vectors_when_disabled(
        self, mock_doc_store_class, temp_db_path
    ):
        mock_doc_store = mock.Mock()
        mock_doc_store_class.return_value = mock_doc_store
    ...
    @mock.patch("talkpipe.search.lancedb.LanceDBDocumentStore")
    @mock.patch("talkpipe.search.lancedb.extract_property")
    def test_add_to_lancedb_yields_original_items(
        self, mock_extract_property, mock_doc_store_class, sample_items, temp_db_path
    ):
        mock_doc_store = mock.Mock()
        mock_doc_store._get_table.return_value = [mock.Mock()]
    ...
    def test_add_to_lancedb_handles_flush_events(
        ...
        class FakeBuffer:
            ...
            def append(self, item):
                self.items.append(item)
                return  # Never auto-flush, only manual flush
...
def test_add_and_search_integration(temp_db_path, sample_items_direction_relevant):
    ...
    search_segment = search_lancedb(
        path=temp_db_path, table_name="test_table", field="vector", limit=2
    )
...
def test_upsert_integration(temp_db_path):
    ...
    add_segment_update = add_to_lancedb(
        path=temp_db_path,
        table_name="test_table",
        vector_field="vector",
        doc_id_field="doc_id",
    ...
    search_segment = search_lancedb(
        path=temp_db_path, table_name="test_table", field="vector", limit=10
    )
...
def test_read_consistency_interval(temp_db_path):
    ...
    assert count_before_wait == 1, (
        f"Reader should not see new document before interval passes, but saw {count_before_wait} documents"
    )
    ...
    assert count_after_wait == 2, (
        f"Reader should see new document after interval passes, but saw {count_after_wait} documents"
    )

    # Verify we can now retrieve the new document
    retrieved_doc = reader_store.get_document(new_doc_id)
    assert retrieved_doc is not None, (
        "New document should be retrievable after consistency interval passes"
    )
...
def test_add_vectors_with_existing_documents(temp_db_path):
    ...
    store.add_vector([1.0, 2.0, 3.0], {"text": "original", "version": "1"}, "doc1")
...
def test_add_vectors_append_mode(temp_db_path):
    ...
    doc_ids = store.add_vectors(
        [([1.0, 2.0], {"text": "a"}, None), ([3.0, 4.0], {"text": "b"}, None)],
        upsert=False,
    )
...
@mock.patch("talkpipe.search.lancedb.LanceDBDocumentStore")
@mock.patch("talkpipe.search.lancedb.extract_property")
def test_add_to_lancedb_appends_without_doc_id_field(
    mock_extract_property, mock_doc_store_class, sample_items, temp_db_path
):
...
@mock.patch("talkpipe.search.lancedb.LanceDBDocumentStore")
@mock.patch("talkpipe.search.lancedb.extract_property")
def test_add_to_lancedb_upserts_with_doc_id_field(
    mock_extract_property, mock_doc_store_class, temp_db_path
):
...
@mock.patch("talkpipe.search.lancedb.LanceDBDocumentStore")
@mock.patch("talkpipe.search.lancedb.extract_property")
def test_add_to_lancedb_periodic_optimize(
    mock_extract_property, mock_doc_store_class, temp_db_path
):
...
@mock.patch("talkpipe.search.lancedb.LanceDBDocumentStore")
@mock.patch("talkpipe.search.lancedb.extract_property")
def test_add_to_lancedb_periodic_optimize_disabled(
    mock_extract_property, mock_doc_store_class, temp_db_path
):
...
class TestDocumentStoreFailuresAreLogged:
    """A LanceDB outage must be distinguishable from an empty database: the
    neutral return values are kept for compatibility, but every swallowed
    exception is logged."""

    def _broken_store(self, temp_db_path, monkeypatch):
        store = LanceDBDocumentStore(temp_db_path)

        def boom():
            raise RuntimeError("lance is down")

        monkeypatch.setattr(store, "_get_table", boom)
        return store

    def test_get_document_logs_failure(self, temp_db_path, monkeypatch, caplog):
        store = self._broken_store(temp_db_path, monkeypatch)
        with caplog.at_level("WARNING", logger="talkpipe.search.lancedb"):
            assert store.get_document("x") is None
        assert "lance is down" in caplog.text
        assert "get_document" in caplog.text

    def test_search_count_list_delete_log_failure(
        self, temp_db_path, monkeypatch, caplog
    ):
        store = self._broken_store(temp_db_path, monkeypatch)
        with caplog.at_level("WARNING", logger="talkpipe.search.lancedb"):
            assert store.vector_search([1.0, 2.0]) == []
            assert store.count() == 0
            assert store.list_ids() == []
            assert store.delete_document("x") is False
        assert caplog.text.count("lance is down") == 4
