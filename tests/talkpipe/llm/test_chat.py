...
from talkpipe.llm.chat import LlmBinaryAnswer, LlmExtractTerms, LLMPrompt, LlmScore
...
def test_context_compaction_falls_back_from_llm(monkeypatch):
    ...
    adapter._messages = [
        {
            "role": "user",
            "content": "A long user message to overflow the token budget.",
        },
...
def test_summary_llm_uses_no_context_completion_hook():
    class SummaryHookAdapter(OllamaPromptAdapter):
        def complete_text_without_context(
            self, prompt, *, model=None, temperature=0.0, max_tokens=None
        ):
...
def test_summary_llm_without_no_context_completion_hook_raises():
    class NoSummaryHookAdapter(OllamaPromptAdapter):
        complete_text_without_context = (
            AbstractLLMPromptAdapter.complete_text_without_context
        )
...
def test_llmprompt_supports_old_adapter_defaults(monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        "talkpipe.llm.chat.getPromptAdapter", lambda _source: OldStyleAdapter
    )
...
def test_openai_chat_guided_generation(requires_openai):
    ...
    chat = OpenAIPromptAdapter(
        "gpt-4.1-nano",
        system_prompt=system_prompt,
        temperature=0.0,
        output_format=TestGuidedGeneration,
    )
...
def test_chat(requires_ollama):
    ...
    chat = LLMPrompt(
        model="llama3.2", source="ollama", multi_turn=False, temperature=0.0
    )
    assert isinstance(chat.chat, OllamaPromptAdapter)
    ...
    chat = compiler.compile(
        'llmPrompt[model="llama3.1", source="ollama", multi_turn=True, temperature=0.0]'
...
def test_chat_set_as(requires_ollama):
    chat = LLMPrompt(
        model="llama3.2", source="ollama", set_as="response", temperature=0.0
    )
    chat = chat.as_function(single_in=True, single_out=True)
    response = chat(
        {"message": "Hello.  My name is Inigo Montoya.  You killed my father."}
    )
...
def test_chat_ollama_custom_host(requires_ollama, monkeypatch):
    ...
    # Test that the chat works with custom URL. The mock will prevent actual
    # connection, but we verified the URL was passed correctly.
...
def test_llmextractterms(requires_ollama):
    system_prompt = (
        """Extract the names of people from the following text in lower case."""
    )
    llmextractterms = LlmExtractTerms(
        system_prompt=system_prompt,
        multi_turn=False,
        temperature=0.0,
        model="llama3.2",
...
def test_llmbinaryanswer(requires_ollama):
    ...
    llmbinaryanswer = LlmBinaryAnswer(
        system_prompt=system_prompt,
        multi_turn=False,
        temperature=0.0,
        model="llama3.2",
