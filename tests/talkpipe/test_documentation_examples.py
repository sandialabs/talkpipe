...
from talkpipe.pipe import basic, io
...
def test_qs_example_1a(capsys):
    ...
    pipeline = (
        io.echo(data="1|2|hello|3", delimiter="|")
        | basic.Cast(cast_type="int")
        | io.Print()
    )
...
def test_load_jsonl_example(tmpdir):
    ...
    ssp = compiler.compile("| readJsonl | toDataFrame")
...
def test_chat_function_example(requires_ollama):
    ...
    f = chat.LLMPrompt(model="llama3.2", source="ollama", multi_turn=True).as_function(
    ...
    f = (
        chat.LLMPrompt(
            model="llama3.2", source="ollama", multi_turn=True, pass_prompts=True
        )
        | io.Print()
        | compiler.Accum(reset=False)
    )
    f = f.as_function(single_in=True, single_out=False)
    f("Good afternoon.  My name is Bob!")
    result = list(f("What is my name?"))
