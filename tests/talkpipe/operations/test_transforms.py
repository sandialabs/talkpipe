...
def test_regex_replace():
    ...
    # Nested fields are rejected when the segment runs, not when it is built
    op = transforms.regex_replace(pattern="a", replacement="b", field="x.2")
    op = op.as_function(single_in=True, single_out=True)
    with pytest.raises(ValueError, match="Nested fields are not supported"):
...
def test_makelists_combinations():
    ...
    ml = transforms.MakeLists(
        num_items=3, cumulative=True, field="data", ignoreNone=True
    )
...
def test_makelists_invalid_parameters():
    ...
    with pytest.raises(
        ValueError, match="num_items must be None or a positive integer"
    ):
