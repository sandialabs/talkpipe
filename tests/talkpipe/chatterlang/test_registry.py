...
class TestHybridRegistryLazyMode:
    ...
    def test_registry_respects_lazy_import_parameter_true(self):
        ...
        test_registry = registry.HybridRegistry(
            entry_point_group="test.group", lazy_import=True
        )
    ...
    def test_registry_respects_lazy_import_parameter_false(self):
        ...
        test_registry = registry.HybridRegistry(
            entry_point_group="test.group", lazy_import=False
        )
...
class TestRegistryAllProperty:
    ...
    def test_all_always_returns_everything_lazy_mode(self):
        ...
        test_registry._load_all_entry_points = MagicMock(
            wraps=test_registry._load_all_entry_points
        )
    ...
    def test_neither_mode_loads_at_init(self):
        ...
        with patch.object(
            registry.HybridRegistry, "_load_all_entry_points"
        ) as mock_load:
            # Test eager mode
...
class TestEntryPointCollisionDetection:
    ...
    def test_collision_detection_raises_error(self):
        ...
        with patch("importlib.metadata.entry_points", return_value=mock_eps_result):
            test_registry = registry.HybridRegistry(
                entry_point_group="test.segments", lazy_import=True
            )
    ...
    def test_collision_detection_with_multiple_conflicts(self):
        ...
        mock_eps_result = MagicMock()
        mock_eps_result.select = MagicMock(
            return_value=[mock_ep1, mock_ep2, mock_ep3, mock_ep4]
        )
...
def test_registry_concurrent_get_and_register_is_safe(caplog):
    """Concurrent lookups and registrations must not error or warn about duplicates."""
    import logging
    import threading

    from talkpipe.chatterlang.registry import HybridRegistry

    reg: HybridRegistry = HybridRegistry(entry_point_group="talkpipe.segments")
    ...
    def worker(i: int) -> None:
        try:
            barrier.wait()
            for _ in range(20):
                reg.register(Dummy, "concurrent_dummy")  # same object, same name
                reg.get("concurrent_dummy")
                reg.get("print")  # entry-point path
                _ = reg.available_names
                if i % 2:
                    reg.invalidate_cache()
        except BaseException as e:
            errors.append(e)

    with caplog.at_level(logging.WARNING, logger="talkpipe.chatterlang.registry"):
    ...
    assert not errors
    assert reg.get("concurrent_dummy") is Dummy
    assert not [r for r in caplog.records if "already registered" in r.getMessage()]
    assert hasattr(reg, "_lock")


class TestDeprecatedAliases:
    def test_deprecated_alias_warns_and_resolves(self):
        with pytest.warns(DeprecationWarning, match="addToLanceDB"):
            aliased = registry.segment_registry.get("addToLancDB")
        canonical = registry.segment_registry.get("addToLanceDB")
        assert aliased is canonical

    def test_file_to_text_alias_warns_and_resolves(self):
        with pytest.warns(DeprecationWarning, match="readFile"):
            aliased = registry.segment_registry.get("fileToText")
        assert aliased is registry.segment_registry.get("readFile")
    ...
    def test_canonical_name_does_not_warn(self):
        ...
        with warnings.catch_warnings():
            ...
            registry.segment_registry.get("readFile")

    def test_deprecated_aliases_hidden_from_available_names(self):
        names = registry.segment_registry.available_names
        assert "addToLancDB" not in names
        assert "fileToText" not in names
