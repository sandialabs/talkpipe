...
\node[box] (flow)                             {FLOW CONTROL};
\node[box, below=0cm of flow] (data)          {DATA MANIPULATION};
\node[box, below=0cm of data] (llm)           {LLM PRIMITIVES};
\node[box, below=0cm of llm] (store)          {DATA STORAGE\\\scriptsize (e.g., full text indexing\\\scriptsize vector database)};
...
\begin{pgfonlayer}{background}
  \node[dockerlayer, fit=(app)(store)] (docker-bg) {};
\end{pgfonlayer}

% Key Functionality background box (middle layer)
\begin{pgfonlayer}{docker}
  \node[keylayer, fit=(flow)(data)(llm)(store)] (key-bg) {};
\end{pgfonlayer}
