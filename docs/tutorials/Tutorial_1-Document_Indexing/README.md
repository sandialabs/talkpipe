...
- **Step 1 only**: an LLM provider. As written, the script uses [Ollama](https://ollama.com/download) with the `llama3.2` model, either installed locally or on a remote server you point TalkPipe at with `export TALKPIPE_OLLAMA_SERVER_URL=http://your-ollama-host:11434` — substitute your server's address, e.g. `http://192.168.1.42:11434` (the model must be pulled on that server). OpenAI or Anthropic work instead; see [Using a Different LLM Provider](#using-a-different-llm-provider)
...
Step 1's script specifies `source="ollama"`, but nothing in the pipeline is tied to Ollama: switching is a parameter change on the LLM segments, not a rewrite. Each segment takes its own `source`/`model`, so different segments can even use different providers. `pip install "talkpipe[all]"` brings in every provider integration (Ollama, OpenAI, Anthropic) up front, so you can switch or mix providers without installing anything else; otherwise install the provider's own extra. To use OpenAI, for example: `pip install "talkpipe[openai]"`, set `OPENAI_API_KEY` in your environment, then edit `Step_1_CreateSyntheticData.script` and change both `llmPrompt` segments to `source="openai", model="gpt-4o"`. Steps 2 and 3 are keyword indexing and search, with no LLM calls, so they need no provider at all. See [LLM providers](../../guides/model-and-source-configuration.md#llm-providers) for every supported provider, what each needs, and how to set defaults so scripts can omit `model`/`source` entirely.
