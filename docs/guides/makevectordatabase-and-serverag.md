...
Run `makevectordatabase` and `serverag` in containers with the [TalkPipe image](https://github.com/sandialabs/talkpipe/pkgs/container/talkpipe). For **where to pull images**, **release tags** (for example pinning a version), and **multi-platform** (amd64/arm64) behavior, see **[Container images](container-images.md)**. The commands work the same under **Docker** and **Podman**—replace `docker` with `podman` if you use Podman. For a custom setup (e.g., extra Python packages or a fixed default command), create a separate Dockerfile based on that image.
