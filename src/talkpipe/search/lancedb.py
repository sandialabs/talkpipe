...
class LanceDBDocumentStore(DocumentStore, VectorAddable, VectorSearchable):
    ...
    def get_document(self, doc_id: DocID) -> Document | None:
        ...
        try:
            ...
            logger.warning(
                f"get_document({doc_id!r}) failed on table '{self.table_name}': {e}"
            )
            return None
    ...
    def vector_search(self, vector: VectorLike, limit: int = 10) -> list[SearchResult]:
        ...
        try:
            ...
            logger.warning(f"vector_search failed on table '{self.table_name}': {e}")
            return []
    ...
    def delete_document(self, doc_id: DocID) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"delete_document({doc_id!r}) failed on table '{self.table_name}': {e}"
            )
            return False
    ...
    def update_document(
        ...
        try:
            ...
            logger.warning(
                f"update_document({doc_id!r}) failed on table '{self.table_name}': {e}"
            )
            return False
    ...
    def count(self) -> int:
        ...
        try:
            ...
            logger.warning(f"count failed on table '{self.table_name}': {e}")
            return 0
    ...
    def list_ids(self) -> list[DocID]:
        ...
        try:
            ...
            logger.warning(f"list_ids failed on table '{self.table_name}': {e}")
            return []
