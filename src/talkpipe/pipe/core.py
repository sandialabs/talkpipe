...
class AbstractSegment(ABC, HasRuntimeComponent, Generic[T, U]):
    ...
    def transform(
        self,
        input_iter: Annotated[Iterable[T], "An iterable of input items to process"],
    ) -> Iterator[U]:
    ...
    def __call__(self, input_iter: Iterable[T] | None = None) -> Iterator[U]:
    ...
    def as_function(
        self,
        single_in: Annotated[
            bool, "If True, the function will expect a single input argument."
        ] = False,
    ...
    def decorator(func: Callable[P, Iterable[U]]) -> type[AbstractSource[U]]:
        class ParameterizedSource(AbstractSource[U]):
            def __init__(self, *init_args: Any, **init_kwargs: Any) -> None:
                ...
                process_metadata = init_kwargs.pop(
                    "process_metadata", decorator_kwargs.get("process_metadata", False)
                )
                super().__init__(process_metadata=process_metadata)
                # Create merged kwargs without process_metadata (it's already handled above)
                merged_kwargs = {
                    k: v
                    for k, v in {**decorator_kwargs, **init_kwargs}.items()
                    if k != "process_metadata"
                }
    ...
    **decorator_kwargs: Annotated[Any, "Keyword arguments for the operation"],
    ...
    def decorator(
        ...
        class ParameterizedSegment(AbstractSegment[T, U]):
            ...
            def transform(self, input_iter: Iterable[T]) -> Iterator[U]:
                return iter(self._func(input_iter))
...
class AbstractFieldSegment(AbstractSegment[T, U]):
    ...
    def transform(self, input_iter: Iterable[T]) -> Iterator[Any]:
        ...
        for item in input_iter:
            ...
            value = (
                data_manipulation.extract_property(item, self.field)
                if self.field
                else item
            )
...
class Script(AbstractSegment[Any, Any]):
    ...
    def transform(
        self,
        initial_input: Annotated[Iterable[Any] | None, "The initial input data"] = None,
    ) -> Iterator[Any]:
...
class Loop(AbstractSegment[Any, Any]):
    ...
    def transform(self, initial_input: Iterable[Any] | None = None) -> Iterator[Any]:
