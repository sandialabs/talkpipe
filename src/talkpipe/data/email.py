...
def send_email(
    ...
    Args:
        ...
        timeout (float, optional): Socket timeout in seconds. Defaults to the
            ``email_timeout`` config value, or 30 seconds if unset.
    Raises:
    ...
    try:
        ...
        smtp_timeout = resolve_timeout(timeout, EMAIL_TIMEOUT, DEFAULT_EMAIL_TIMEOUT)
...
def sendEmail(
    ...
    timeout: Annotated[
        float | None,
        "SMTP socket timeout in seconds; defaults to the email_timeout config value (30 if unset)",
    ] = None,
) -> Iterator[Any]:
    ...
    config = get_config()
    logger.debug(f"Loaded config: {redact(config)}")
...
def fetch_emails(
    ...
    """
    imap_timeout = resolve_timeout(timeout, EMAIL_TIMEOUT, DEFAULT_EMAIL_TIMEOUT)
    mail: Any = None
    try:
        ...
        # Release the connection on every exit path: normal completion, an
        # error, or the consumer abandoning the generator (e.g. firstN).
        if mail is not None:
            for step in (mail.close, mail.logout):
                try:
                    step()
                except Exception as exc:  # best-effort cleanup
                    logger.debug(f"Ignoring IMAP cleanup error: {exc}")
...
def readEmail(
    ...
    timeout: Annotated[
        float | None,
        "IMAP socket timeout in seconds; defaults to the email_timeout config value (30 if unset)",
    ] = None,
) -> Iterator[dict[str, Any]]:
