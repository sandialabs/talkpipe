...
from talkpipe.util.config import get_config, parse_key_value_str, resolve_timeout
...
class MongoInsert(core.AbstractSegment[Any, Any]):
    ...
    def __init__(
        self,
        connection_string: Annotated[str | None, "MongoDB connection string"] = None,
        database: Annotated[str | None, "Name of the MongoDB database to use"] = None,
        collection: Annotated[
            str | None, "Name of the MongoDB collection to use"
        ] = None,
        field: Annotated[str, "Field to extract from each item for insertion"] = "_",
        fields: Annotated[
            str | None, "Comma-separated list of fields to extract"
        ] = None,
        set_as: Annotated[
            str | None, "Field name to add MongoDB insertion result to item"
        ] = None,
        create_index: Annotated[str | None, "Field to create an index on"] = None,
        unique_index: Annotated[bool, "Whether to create a unique index"] = False,
        timeout: Annotated[
            float | None,
            "Server selection / connect timeout in seconds; defaults to the mongo_timeout config value (30 if unset)",
        ] = None,
    ):
        super().__init__()
        ...
        self.timeout = resolve_timeout(timeout, MONGO_TIMEOUT, DEFAULT_MONGO_TIMEOUT)
        ...
        self.parsed_fields: dict[str, str] | None = None
    ...
    def _ensure_connection(self) -> Collection[Any]:
        ...
        if self._collection is None:
            ...
            if self.create_index:
                logger.debug(
                    f"Creating index on {self.create_index} (unique={self.unique_index})"
                )
                collection.create_index(self.create_index, unique=self.unique_index)
                ...
                try:
                    ...
                    if self.parsed_fields is not None:
                        ...
                        for source_field, target_field in self.parsed_fields.items():
                            try:
                                field_value = extract_property(
                                    item, source_field, fail_on_missing=True
                                )
                                data_to_insert[target_field] = field_value
                            except (AttributeError, KeyError) as e:
                                logger.warning(
                                    f"Could not extract field '{source_field}': {e}"
                                )
                    elif self.field == "_":
                    ...
                    if data_to_insert is None or (
                        isinstance(data_to_insert, dict) and len(data_to_insert) == 0
                    ):
                        logger.warning("Skipping empty document or None value")
                        yield item
                        continue
                    ...
                    # Insert into MongoDB
                    logger.debug(
                        f"Inserting a document with {len(data_to_insert)} fields "
                        f"into {self.database_name}.{self.collection_name}"
                    )
...
class MongoSearch(core.AbstractSegment[Any, Any]):
    ...
    def __init__(
        ...
        project: Annotated[
            str | None, "JSON string defining projection for returned documents"
        ] = None,
        sort: Annotated[str | None, "JSON string defining sort order"] = None,
        limit: Annotated[int, "Maximum number of results to return per query"] = 0,
        skip: Annotated[int, "Number of documents to skip"] = 0,
        set_as: Annotated[
            str | None, "Field name to add MongoDB results to incoming item"
        ] = None,
        timeout: Annotated[
            float | None,
            "Server selection / connect timeout in seconds; defaults to the mongo_timeout config value (30 if unset)",
    ...
    def transform(self, input_iter: Iterable[Any]) -> Iterator[Any]:
        ...
        try:
            for item in input_iter:
                ...
                logger.debug(
                    f"Querying {self.database_name}.{self.collection_name}: {actual_query}"
                )
