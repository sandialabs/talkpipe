...
def run_command(command: str) -> Iterator[str]:
    ...
    Security note: the checks below (no shell, no shell metacharacters, no
    path traversal, an allow-list of program names) guard against accidental
    misuse in trusted scripts. They are not a sandbox: several allowed
    programs (``python``, ``pip``, ``find``, ``awk``, ...) can run arbitrary
    code, so treat any script that uses ``exec`` as fully trusted. See
    ...
    try:
        ...
        if process.returncode != 0:
            ...
            logger.error(
                f"Command failed with return code {process.returncode}: {stderr_output}"
            )
...
def _validate_command_security(command: str) -> None:
    ...
    for pattern in dangerous_patterns:
        if pattern in command:
            raise SecurityError(
                f"Security violation: Command contains dangerous pattern '{pattern}'"
            )

    # Check for path traversal attempts
    if ".." in command or "~/" in command:
        raise SecurityError(
            "Security violation: Command contains path traversal patterns"
        )
    ...
    for path in sensitive_paths:
        if path in command_lower:
            raise SecurityError(
                f"Security violation: Command attempts to access sensitive path '{path}'"
            )
...
def _validate_base_command(base_command: str) -> None:
    ...
    if command_name not in allowed_commands:
        ...
        logger.warning(
            f"Attempted execution of non-whitelisted command: {base_command}"
        )
        raise SecurityError(
            f"Security violation: Command '{command_name}' is not in the allowed list"
        )
