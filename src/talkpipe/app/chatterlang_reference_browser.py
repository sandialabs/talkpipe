...
class TalkPipeDoc:
    ...
    def __init__(
        ...
        self.name = name
        self.chatterlang_names = (
            chatterlang_names  # List of all names for this component
        )
...
class TalkPipeBrowser:
    ...
    def load_components(self):
        ...
        class_to_names: dict[Any, list[str]] = {}
        class_to_type: dict[Any, str] = {}
        ...
        for cls, names in class_to_names.items():
            ...
            component_info = extract_component_info(
                primary_name, cls, class_to_type[cls]
            )
            if component_info:
    ...
    def _load_component_from_info(
        ...
        try:
            ...
            for param in component_info.parameters:
                ...
                if param.annotation:
                    max_type_width = max(
                        max_type_width, len(_one_line(param.annotation))
                    )
                if param.default:
                    max_default_width = max(
                        max_default_width, len(_one_line(param.default))
                    )
    ...
    def _list_module_components(self, module_name: str) -> None:
        ...
        if module_name not in self.modules:
            # Try partial match
            matches = [mod for mod in self.modules if module_name in mod]
            if len(matches) == 1:
    ...
    def _show_component(self, component_name: str) -> None:
        ...
        if not component:
            matches = [
                name
                for name in self.name_to_primary
        ...
        if component.parameters:
            print("\nParameters:")
            print("-" * 11)
            for param_info in component.parameters.values():
                print(f"  {param_info}")
    ...
    def _search_components(self, search_term: str) -> None:
        ...
        for component in self.components.values():
            ...
            name_match = any(
                search_lower in name.lower() for name in component.chatterlang_names
            )
            if (
                name_match
                or search_lower in component.name.lower()
                or search_lower in component.docstring.lower()
            ):
...
def main():
    ...
    try:
        ...
        print(
            f"Loaded {len(browser.components)} components from {len(browser.modules)} modules"
        )
        browser.run()
    except Exception as e:
