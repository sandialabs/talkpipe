...
class AnalyzedItem:
    ...
    chatterlang_name: str | None = None
...
def analyze_registered_items() -> list[AnalyzedItem]:
    ...
    class_to_names: dict[Any, dict[str, Any]] = {}
...
def convert_component_info_to_analyzed_item(
...
"""
    ...
    for item_type in sorted(items_by_type.keys()):
        ...
        for item in type_items:
            ...
            html_content += "<tr>\n"
            html_content += '  <td class="toc-chatterlang">'
            if item.chatterlang_name:
                html_content += (
                    f'<a href="#{item_id}">{html.escape(item.chatterlang_name)}</a>'
                )
            html_content += "</td>\n"
            ...
            # Description column
            html_content += '  <td class="toc-description">'
            if first_doc_line:
...
"""
    ...
    for item_type in sorted(items_by_type.keys()):
        ...
        type_id = sanitize_id(item_type)
        html_content += (
            f'<div class="type-header" id="{type_id}">{html.escape(item_type)}</div>\n'
        )
        ...
        for item in type_items:
            ...
            if item.docstring.strip():
                ...
                html_content += (
                    f"  <div><pre>{html.escape(item.docstring)}</pre></div>\n"
                )
    ...
    for item_type in sorted(items_by_type.keys()):
        ...
        for item in type_items:
            ...
            if item.docstring.strip():
                ...
                lines.extend(
                    f"    {ds_line}" for ds_line in item.docstring.splitlines()
                )
            else:
...
def go() -> None:
    ...
    main(
        args.html_out or "./chatterlang_ref.html",
        args.text_out or "./chatterlang_ref.txt",
    )
