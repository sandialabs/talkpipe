...
def scan_file(
    ...
    try:
        ...
        segments.extend(
            (reg_name, obj_name, module_path)
            for reg_name, obj_name in factory_finder.segments
        )

        return {"sources": sources, "segments": segments}
...
def scan_directory(
    directory: Path, package_name: str
) -> dict[str, list[tuple[str, str, str]]]:
    """
    ...
    return {"sources": all_sources, "segments": all_segments}


def generate_toml_section(
    sources: list[tuple[str, str, str]], segments: list[tuple[str, str, str]]
) -> str:
    """
...
def main():
    ...
    parser.add_argument(
        "--output", type=Path, help="Output file (default: print to stdout)"
    )
