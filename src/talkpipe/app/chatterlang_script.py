...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Run a talkpipe script provided on the command line."
    )
    parser.add_argument(
        "--script",
        type=str,
        required=True,
        help="The talkpipe script to run: file path, configuration key, or inline script content.",
    )
    parser.add_argument(
        "--load-module",
        action="append",
    ...
    parser.add_argument(
        "--logger_files",
        type=str,
        help="Logger files in format 'logger:file,logger:file,...'",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
        help="Show the full Python traceback on script compile and runtime errors (for debugging), instead of just the error message.",
    )
    ...
    if constants:
        ...
        logger.info(
            f"Added command-line values to configuration: {list(constants.keys())}"
        )
