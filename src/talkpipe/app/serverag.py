...
def main() -> None:
    ...
    parser.add_argument(
        "--path", type=str, required=True, help="Path to the LanceDB database"
    )
    ...
    parser.add_argument(
        "--completion_model",
        type=str,
        help="LLM model to use for completion (defaults to config)",
    )
    parser.add_argument(
        "--completion_source",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="docs",
        help='Name of the table to query (default: "docs")',
    )
    parser.add_argument(
        "--prompt_directive",
        type=str,
    ...
    parser.add_argument(
        "--system_prompt", type=str, help="System prompt for the completion LLM"
    )
    parser.add_argument(
        "--memory_mode",
        type=str,
    ...
    # Server settings
    parser.add_argument(
        "-p", "--port", type=int, default=2026, help="Port to listen on (default: 2026)"
    )
    parser.add_argument(
        "-o", "--host", default="localhost", help="Host to bind to (default: localhost)"
    )
    parser.add_argument(
        "--title", default="Talkpipe RAG Server", help="Title for the web UI"
    )
    parser.add_argument("--api_key", help="Set API key for authentication")
    parser.add_argument(
        "--require_auth", action="store_true", help="Require API key authentication"
    )
    ...
    parser.add_argument(
        ...
        "--interactive",
        action="store_true",
        help="Run in interactive CLI REPL mode instead of starting a web server",
    )
    ...
    try:
        rag_kwargs = {
            ...
            or config.get(TALKPIPE_EMBEDDING_MODEL_NAME),
            "embedding_source": args.embedding_source
            or config.get(TALKPIPE_EMBEDDING_MODEL_SOURCE),
            "completion_model": args.completion_model
            or config.get(TALKPIPE_MODEL_NAME),
    ...
    if args.interactive:
        ...
        for item in cli_pipeline():
            ...
            content = (
                item.get("answer", str(item)) if isinstance(item, dict) else str(item)
            )
            print("\n" + str(content) + "\n")
            print("-" * 50)
        ...
        logger.info(
            f"Starting RAG Server on {args.host}:{args.port} querying {args.path}"
        )
