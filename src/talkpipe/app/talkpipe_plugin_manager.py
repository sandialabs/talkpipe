...
from talkpipe.chatterlang.registry import (
    DEPRECATED_ALIASES,
    input_registry,
    segment_registry,
)
...
def main():
    ...
    if args.list:
        ...
        segments = {
            name: target
            for name, target in segment_registry.list_entry_points().items()
            if name not in DEPRECATED_ALIASES
        }
        sources = {
            name: target
            for name, target in input_registry.list_entry_points().items()
            if name not in DEPRECATED_ALIASES
        }
